...
//STEP7- extracting enter and exit selection
...
function remove(){
  ...
  //update yScale(yDomain)
  yMax = d3.max(data, function(d){return d.value});
  yDomain = [0, yMax+yMax*0.1];
  yScale.domain(yDomain);

    exitingElements.select("rect").transition().duration(1000)
      .attr("fill", "#F27294")//this seems do not work, why
      .attr('height',function(d, i){
        return 0
      })
      .attr('y',function(d, i){
  ...
  // extract the entering and existing elments
  enteringElements = elementsForPage.enter();
  exitingElements = elementsForPage.exit();
  elementsForPage.transition().duration(1000).attr("transform", function(d, i){
      return "translate("+ xScale(d.key)+ "," + (h - padding) + ")"
...
function shuffleData(){
  ...
  // update data and x scale domain
  allNames = data.map(function(d){return d.key});
  xScale.domain(allNames);
...
//*** BUTTON
//Shuffle and change random color
function shuffleAndChangeColor(){
  shuffleDatapoints();
...
function randomColor(){
  let color = ["DarkSeaGreen","DarkOliveGreen","OliveDrab","YellowGreen","MediumSeaGreen","LimeGreen","LightGreen","ForestGreen","SeaGreen","green"];
...
document.getElementById("buttonF").addEventListener("click",shuffleAndChangeColor)